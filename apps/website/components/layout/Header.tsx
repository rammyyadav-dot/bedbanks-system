...
export function Header({ agentHref }: { agentHref: string }) {
  ...
  return <header className="site-header">
    <div className="container-wide header-inner">
      ...
      <div className="header-actions">
        <a href={agentHref} className="signin-link" onClick={() => trackWebsiteEvent('portal_link_selected', { portal: 'agent' })}>Portal sign in</a>
    ...
    {open && <div className="mobile-nav-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false) }}>
      <div id="mobile-navigation" className="mobile-nav" role="dialog" aria-modal="true" aria-label="Mobile navigation">
        ...
        <div className="mobile-nav-actions"><a href={agentHref} className="button button-secondary" onClick={() => trackWebsiteEvent('portal_link_selected', { portal: 'agent' })}>Portal sign in</a><Link href="/request-demo" className="button button-primary" onClick={() => trackWebsiteEvent('primary_cta_selected', { placement: 'mobile_navigation' })}>Request a demo</Link></div>
      </div>
    </div>}
  </header>
}
