...
export function PropertiesView({ properties }: { properties: Property[] }) {
  ...
  return (
    <div className="supplier-page">
      <PageHeader eyebrow="Supply portfolio" title="Properties" description="Manage property content, contracting readiness, inventory health and channel mapping from one operational view." actions={<Link className="btn btn-primary" href="/properties/new"><Plus size={13} /> Add property</Link>} />
      ...
      <section className="panel">
        <div className="table-wrap">
          <table className="data-table property-table">
            ...
            <tbody>{filtered.map((property) => <tr key={property.id}>
              <td><div className="property-cell"><span>{property.stars}★</span><div><Link href={`/properties/${property.id}`}>{property.name}</Link><small>{property.id} · {property.destination}, {property.country}</small></div></div></td>
        ...
        {filtered.length === 0 && <div className="empty-state"><Search size={24} /><strong>No matching properties</strong><span>Try a broader property name, destination or reference.</span></div>}
        <div className="panel-pagination"><span>Showing {filtered.length} of 12 properties</span><button disabled>Previous</button><button>Next <ArrowRight size={11} /></button></div>
      </section>
    </div>
  )
}
