...
export class AgentRbacGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!formalPermissions.includes(required) && !legacyAllowed) {
      await this.prisma.auditEvent.create({ data: { actorType: 'USER', action: 'permission.denied', entityType: 'permission', entityId: required, payload: { tenantId }, userId: identity.user.id } }).catch(() => undefined)
