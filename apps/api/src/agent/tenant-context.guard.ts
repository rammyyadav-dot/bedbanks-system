...
export class TenantContextGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (!membership || membership.tenant.status !== 'ACTIVE') {
      await this.prisma.auditEvent.create({ data: { actorType: identity ? 'USER' : 'SYSTEM', action: 'tenant.access.denied', entityType: 'tenant', entityId: tenantId ?? 'unknown', payload: { reason: 'inactive_or_missing_membership' }, userId: identity?.user.id } }).catch(() => undefined)
      throw new ForbiddenException('Access denied')
    }
    await this.prisma.auditEvent.create({ data: { actorType: 'USER', action: 'tenant.context.selected', entityType: 'tenant', entityId: tenantId, payload: { source: 'request_context' }, userId: identity.user.id } }).catch(() => undefined)
